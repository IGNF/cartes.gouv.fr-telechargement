...
function Menu({ selectedDalles, title }: { selectedDalles: any[], title: string }) {
  ...
  return (
    ...
    <><Accordion label="Dalles séléctionné">
          <ul>
            {selectedDalles.map((dalle, index) => (
              <li key={index}>
                <strong>Nom :</strong><br />
                <Download details="" label={dalle.name} linkProps={{ href: dalle.url }}></Download>
                {/* Ajoutez d'autres propriétés ici si nécessaire */}
              </li>
            ))}
          </ul>
        </Accordion><Button onClick={onDownload}>
            Label button
          </Button></>
      ) : (
